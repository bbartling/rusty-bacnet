...
pub(super) fn encode(request: &AuditNotificationRequest, buf: &mut BytesMut) -> Result<(), Error> {
    ...
    // Encode into scratch storage so validation failures never partially append
    // a service payload to the caller's APDU buffer.
    let mut encoded = BytesMut::new();
    tags::encode_opening_tag(&mut encoded, NOTIFICATIONS_TAG);
    for notification in &request.notifications {
        encode_notification(notification, &mut encoded)?;
    }
...
pub(super) fn decode(data: &[u8]) -> Result<AuditNotificationRequest, Error> {
    ...
    while offset < body.len() {
        ...
        let (notification, next) = decode_notification(body, offset)?;
        if next <= offset {
            return Err(Error::decoding(
                body_start + offset,

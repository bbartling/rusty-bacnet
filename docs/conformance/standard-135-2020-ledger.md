...
| `BACNET-AB-SC-DATA-ATTRIBUTES` | Annex AB.3.4 | P0 | `implementation-present-needs-conformance-tests` | Native receive-side, outbound send, router local-delivery, router forwarding, unsupported Must Understand fail-closed, and WASM/browser parity evidence now covers BACnet/SC Encapsulated-NPDU Data Options. Receive paths convert supported or non-Must-Understand Data Options into generic transport or WASM `DataAttribute` values, preserving option type, Must Understand flag, and option data on `ReceivedNpdu` or `ReceivedScNpdu`; `NetworkLayer` exposes those attributes on `ReceivedApdu` after NPDU decode; router local-delivery paths retain inbound attributes; router forwarding attaches inbound attributes to forwarded unicast and broadcast send requests; and non-SC transports emit empty `data_attributes` on receive. Native SC and the WASM receive loop treat Secure Path Data Option type 1 as understood, reject other Must Understand Data Options before NPDU delivery, return BVLC-Result NAK COMMUNICATION/HEADER_NOT_UNDERSTOOD with the offending option marker for non-broadcast traffic, drop broadcast traffic without a result, and preserve unsupported non-Must-Understand Data Options as attributes. Outbound paths add attribute-aware `TransportPort` and `NetworkLayer` send helpers, keep non-SC transports on default ignore-on-send behavior, and encode SC `DataAttribute` values as Data Options while validating option type `1..31`, the 64-option cap, and u16 option-data lengths before writing the BVLC-SC frame. The WASM/browser client exposes incoming attributes through `onNpdu` metadata, accepts raw `sendNpduWithDataAttributes` arrays, emits hub-bound Encapsulated-NPDU frames with Destination VMAC and no Originating VMAC, and retains peer Max-NPDU-Length/Max-BVLC-Length send-path checks. Router tests cover forwarded unicast, forwarded broadcast, and loopback SC-to-SC forwarding preserving Data Options on the outbound SC frame. Remaining work is live browser/WebSocket smoke coverage and formal conformance-test evidence. |

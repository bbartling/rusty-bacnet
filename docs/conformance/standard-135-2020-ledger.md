...
| `BACNET-AB-SC-DATA-ATTRIBUTES` | Annex AB.3.4 | P0 | `implementation-present-needs-conformance-tests` | Native receive-side evidence now converts BACnet/SC Encapsulated-NPDU Data Options into generic transport `DataAttribute` values, preserving option type, Must Understand flag, and option data on `ReceivedNpdu`; `NetworkLayer` exposes them on `ReceivedApdu` after NPDU decode, router local-delivery paths retain inbound attributes, and non-SC transports emit empty `data_attributes`. Remaining gaps include outbound SC `data_attributes` to Data Options support, SC-to-SC router forwarding preservation, unsupported Must Understand data-option fail-closed policy, and WASM/browser parity. |

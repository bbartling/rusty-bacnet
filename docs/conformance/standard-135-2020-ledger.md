...
| `BACNET-AB-SC-DATA-ATTRIBUTES` | Annex AB.3.4 | P0 | `implementation-present-needs-conformance-tests` | Native receive-side and outbound send evidence now covers BACnet/SC Encapsulated-NPDU Data Options. Receive paths convert Data Options into generic transport `DataAttribute` values, preserving option type, Must Understand flag, and option data on `ReceivedNpdu`; `NetworkLayer` exposes them on `ReceivedApdu` after NPDU decode; router local-delivery paths retain inbound attributes; and non-SC transports emit empty `data_attributes` on receive. Outbound paths add attribute-aware `TransportPort` and `NetworkLayer` send helpers, keep non-SC transports on default ignore-on-send behavior, and encode SC `DataAttribute` values as Data Options while validating option type `1..31`, the 64-option cap, and u16 option-data lengths before writing the BVLC-SC frame. Remaining gaps include SC-to-SC router forwarding preservation, unsupported Must Understand data-option fail-closed policy, and WASM/browser parity. |

...
| `BACNET-AB-SC-DATA-ATTRIBUTES` | Annex AB.3.4 | P0 | `implementation-present-needs-conformance-tests` | Native receive-side, outbound send, router local-delivery, and router forwarding evidence now covers BACnet/SC Encapsulated-NPDU Data Options. Receive paths convert Data Options into generic transport `DataAttribute` values, preserving option type, Must Understand flag, and option data on `ReceivedNpdu`; `NetworkLayer` exposes them on `ReceivedApdu` after NPDU decode; router local-delivery paths retain inbound attributes; router forwarding attaches inbound attributes to forwarded unicast and broadcast send requests; and non-SC transports emit empty `data_attributes` on receive. Outbound paths add attribute-aware `TransportPort` and `NetworkLayer` send helpers, keep non-SC transports on default ignore-on-send behavior, and encode SC `DataAttribute` values as Data Options while validating option type `1..31`, the 64-option cap, and u16 option-data lengths before writing the BVLC-SC frame. New router tests cover forwarded unicast, forwarded broadcast, and loopback SC-to-SC forwarding preserving Data Options on the outbound SC frame. Remaining gaps include unsupported Must Understand data-option fail-closed policy and WASM/browser parity. |
